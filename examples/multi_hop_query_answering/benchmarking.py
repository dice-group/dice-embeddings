...
# Import necessary modules from DICEE library
...
# Flag to track whether queries have been saved
queries_saved = False

# Loop over different knowledge graph embedding models
for kge_name in ["DistMult", "ComplEx", "Keci", "Pykeen_QuatE", "Pykeen_MuRE"]:
    # (1) Train a model for the current knowledge graph embedding (KGE) method
    args = Namespace()
    args.model = kge_name
    ...
    # Execute the training and obtain reports
    reports = Execute(args).start()
    
    # (2) Load the pretrained model and retrieve training configuration details
    ...
    # (3) Generate multi-hop queries
    #Enter paths for train,val and test files
    qg = QueryGenerator(
        train_path="KGs\UMLS\train.txt",
        val_path="KGs\UMLS\valid.txt",
        test_path="KGs\UMLS\test.txt",
        ent2id=pre_trained_kge.entity_to_idx,
        rel2id=pre_trained_kge.relation_to_idx, seed=1)
    
    # Check if queries and answers have been saved
    if queries_saved is False:
        ...
        queries_and_answers = [(q, qg.get_queries(query_type=q, gen_num=100)) for q in
                               ['1p', '2p', '3p', '2i', '3i', 'pi', 'ip', '2in', '3in', 'pin', 'pni', 'inp', '2u','up']]
        
        # # Load generated queries and answers
    ...
    # Initialize a list to store results for each KGE model
    results_kge = []
    ...
    for query_type, (query_structs_and_queries, easy_answers, false_positives, hard_answers) in queries_and_answers:
        for _, queries in query_structs_and_queries.items():
            try:
                ...
                print(f"Error processing {query_type} queries: {e}")
                
    # Create a DataFrame to store results for each KGE model
    df = pd.DataFrame(results_kge, columns=["Query", "Size", "MRR", "H1", "H3", "H10"])
    data_frames_results.append((kge_name, num_param, runtime, df))

# (5) Print aggregated results for each KGE model
