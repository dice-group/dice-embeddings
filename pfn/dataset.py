...
def _load_real_triples(
    ...
    entity_strings, relation_strings, kg_name)`` tuples — one per KG.  ``triples`` is a list of
    ...
    for root, _dirs, files in os.walk(kg_dir):     # walk the directory tree recursively
        ...
        pools.append((triples, entity_embs, relation_embs, entity_to_triples, entity_to_neighbors,
                      entity_strings, relation_strings, kg_name))
...
class RichSubgraphPrior:
    ...
    def _generate_real_task(
        ...
        **Algorithm outline:**
        ...
        triples, entity_embs, relation_embs, entity_to_triples, entity_to_neighbors, entity_strings, relation_strings, kg_name = random.choice(self.kg_pools)
        ...
        return support_tensor, query_tensor, torch.tensor(float(label)), kg_name
    ...
    def generate_task(
        ...
        kg_name      : str                                     — name of the source KG
        """
...
class RandomSupportPrior:
    ...
    def _generate_real_task(
        ...
        triples, entity_embs, relation_embs, _e2t, _e2n, entity_strings, relation_strings, kg_name = random.choice(self.kg_pools)
        n_ent = entity_embs.shape[0]
...
def build_dataset(
    ...
    for i in range(num_episodes):
        ...
        sup, qry, lbl, kg_name = prior.generate_task(context_size=context_size, verbose=verbose, force_label=force_label)
        sup_mm[i] = sup.numpy()
        qry_mm[i] = qry.numpy()
        lbl_mm[i] = lbl.item()
        kg_counts[kg_name] = kg_counts.get(kg_name, 0) + 1
    ...
    print(f"\n  Episode sources:")
    sorted_kgs = sorted(kg_counts.items(), key=lambda x: x[1], reverse=True)
    for kg_name, count in sorted_kgs:
        pct = 100.0 * count / num_episodes
        print(f"    {kg_name:<30s}  {count:>7,} episodes  ({pct:5.1f}%)")
    print()
