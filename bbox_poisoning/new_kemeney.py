...
def read_triples(path, sep=None, comment_prefix="#"):
    ...
    with open(path, "r", encoding="utf-8") as f:
        for ln, raw in enumerate(f, 1):
            ...
            if len(parts) < 3:
                # skip malformed lines; or log if you enjoy noise
                continue
    ...
    for i in range(n):
        ...
        for p in range(row_start, row_end):
            ...
            if i < j:
                ...
                denom = 1.0 - alpha
                if abs(denom) < 1e-14:
                    c_r = 1e12  # treat near-bridge as massive damage
                else:
...
def select_edges_by_kemeny_only(triples, undirected_pairs, pair_to_indices, scores, budget_triples, allow_partial=False):
    """
    ...
    Returns a list of triple indices (into 'triples') to delete.
    """
