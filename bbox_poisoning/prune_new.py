...
def compute_edge_scores_for_neighbor_damage(
    ...
    Candidates:
      - any triple where at least one endpoint is in hubs or their neighbors.

    Score:
      base = 1 / min(deg(u), deg(v))
      if use_backup:
        multiply base by 3 if NO backup path of length <= backup_k exists (excluding direct u-v).
    """
    # hubs + neighbors
...
def main() -> None:
    ...
    if not scored:
        print("No candidate edges touching hubs or their neighbors; nothing to prune.")
        return
    ...
    with open(out_report, "w", encoding="utf-8") as f:
        ...
        for hub in hubs:
            ...
            f.write(f"{hub}\tbefore\t{b.get('num_neighbors', 0.0)}\t{b.get('avg_deg', 0.0)}\t{b.get('median_deg', 0.0)}\t{b.get('num_deg1_or_less', 0.0)}\n")
            f.write(f"{hub}\tafter\t{a.get('num_neighbors', 0.0)}\t{a.get('avg_deg', 0.0)}\t{a.get('median_deg', 0.0)}\t{a.get('num_deg1_or_less', 0.0)}\n")
    ...
    print(f"Candidate edges touching hubs or neighbors: {num_candidates}")
