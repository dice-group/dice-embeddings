...
from math import inf  # you may not strictly need this, but it's here if you want it
...
def compute_edge_scores_for_neighbor_damage(
    ...
    backup_k: int,
    use_backup: bool,
    rel_include: Optional[Set[str]] = None,
    rel_exclude: Optional[Set[str]] = None,
) -> List[Tuple[float, int, str]]:
    """
    Score edges (triples) by how much they hurt neighbor connectivity
    around hubs.

    Candidates:
      - relation r passes rel_include / rel_exclude filters
      - and at least one endpoint is in hubs or their neighbors.

    Score:
      base = 1 / harmonic_mean(deg(u), deg(v))
      if use_backup:
        multiply base by 2 if NO backup path of length <= backup_k exists (excluding direct u-v).
    """
    # hubs + neighbors
    neighbors = neighbors_of_hubs(adj, hubs)
    target_nodes = set(hubs) | neighbors
    ...
    for idx, (h, r, t) in enumerate(triples):
        # relation filters
        if rel_include is not None and r not in rel_include:
            continue
        if rel_exclude and r in rel_exclude:
            continue
        ...
        # harmonic mean of degrees; small if either endpoint has small degree
        harmonic = 2.0 / (1.0 / du + 1.0 / dv)
        base = 1.0 / harmonic
        ...
        if use_backup and backup_k > 0:
            ...
            if has_backup:
                score = base
                reason = f"candidate_harmonic_with_backup-rel:{r}-du:{du}-dv:{dv}"
            else:
                score = 2.0 * base
                reason = f"candidate_harmonic_no_backup-rel:{r}-du:{du}-dv:{dv}"
        else:
            score = base
            reason = f"candidate_harmonic-rel:{r}-du:{du}-dv:{dv}"
...
def main() -> None:
    ...
    ap.add_argument("--remove-frac", type=float, default=0.1,
                    help="Fraction of candidate edges (touching hubs or their neighbors) to remove globally.")
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=None,
        help="If set, ONLY these relations will be considered for scoring/pruning.",
    )
    ap.add_argument(
        "--rel-exclude",
        nargs="*",
        default=None,
        help="Relations to ignore for scoring/pruning (e.g., isa).",
    )

    args = ap.parse_args()

    rel_include: Optional[Set[str]] = set(args.rel_include) if args.rel_include else None
    rel_exclude: Set[str] = set(args.rel_exclude or [])
    ...
    # Neighbor sets in the ORIGINAL graph
    neighbors = neighbors_of_hubs(adj, hub_set)

    # Degree stats BEFORE pruning
    before_stats = compute_neighbor_deg_stats(adj, hub_set, neighbors)

    # Score candidate edges for neighbor damage
    ...
    if not scored:
        print("No candidate edges touching hubs or their neighbors (after relation filters); nothing to prune.")
        return
    ...
    # Build new adjacency for AFTER pruning stats
    ...
    print(f"Candidate edges touching hubs or neighbors (after relation filter): {num_candidates}")
