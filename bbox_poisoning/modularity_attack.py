...
def improve_modularity_reduce_clustering(
    ...
    if verbose:
        print(f"[improve] Starting with budget={budget}, edges={G.number_of_edges()}")
    ...
    if verbose:
        print(f"[improve] Step 0: Q={Q:.6f}, C={C:.6f}")
    ...
    for step in range(1, budget + 1):
        ...
        if not edges:
            if verbose:
                print("[improve] No edges left, stopping.")
            break
                    ...
                    for w in cand_nodes[:5]:
                        ...
                        if (not (Q_new != Q and (Q_new < Q))) and (C_new <= C):
                            ...
                            if verbose:
                                print(
                                    f"[improve] step {step}: "
                                    f"accepted Move A, Q {Q_prev:.6f}->{Q:.6f}, "
                                    f"C {C_prev:.6f}->{C:.6f}"
                                )
                            break
                        else:
                ...
                for w in cand_nodes[:5]:
                    ...
                    if (Q_new >= Q and C_new <= C) or (C_new < C and (Q_new + 1e-6) >= Q):
                        ...
                        if verbose:
                            print(
                                f"[improve] step {step}: "
                                f"accepted Move B, Q {Q_prev:.6f}->{Q:.6f}, "
                                f"C {C_prev:.6f}->{C:.6f}"
                            )
                        break
                    else:
        ...
        if not improved:
            if verbose:
                print(f"[improve] No improving move at step {step}, stopping.")
            break

    if verbose:
        print(f"[improve] Finished. Accepted steps: {len(history)-1}")
    return G, history
...
def reduce_modularity_increase_clustering(
    ...
    if verbose:
        print(f"[ruin] Starting with budget={budget}, edges={G.number_of_edges()}")
    ...
    history = [{"step": 0, "Q": Q, "C": C}]
    if verbose:
        print(f"[ruin] Step 0: Q={Q:.6f}, C={C:.6f}")
    ...
    for step in range(1, budget + 1):
        ...
        if not edges:
            if verbose:
                print("[ruin] No edges left, stopping.")
            break
        ...
        if not improved:
            if verbose:
                print(f"[ruin] No improving move at step {step}, stopping.")
            break

    if verbose:
        print(f"[ruin] Finished. Accepted steps: {len(history)-1}")
    return G, history
...
def compute_diff_triples(original_triples, G_original, G_new):
    ...
    Note:
      We ignore direction and relations for structure, consistent with the graph.
    """
    ...
    # Edges that are newly created structurally (no original triple between endpoints)
    added_edge_pairs = new_edges - orig_edges

    # Represent added edges as (u, v) pairs (no relation info)
    added_edges = []
...
if __name__ == "__main__":
    ...
    DB = "NELL-995-h100"
    path = f"./KGs/{DB}/train.txt"   # adjust to your setup
    ...
    # 1) To increase modularity & reduce clustering:
    ...
    # 2) To reduce modularity & increase clustering:
    ...
    # Save to files
    out_dir = f"./modu/{DB}"
    # make sure the directory exists yourself, or wrap in os.makedirs

    with open(f"{out_dir}/removed.txt", "w") as f:
    ...
    # Added edges don't have an original relation; just save endpoints
    with open(f"{out_dir}/added.txt", "w") as f:
        for u, v in added_edges:
            f.write(f"{u} {v}\n")

    print("\nDiff saved to:")
    print(f"  {out_dir}/removed.txt")
    print(f"  {out_dir}/train.txt")
    print(f"  {out_dir}/added.txt")
