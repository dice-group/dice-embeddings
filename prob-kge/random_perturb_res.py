...
for DB in DBS:
    ...
    with open(out_csv, "a", newline="") as f:
        ...
        for run_model in MODELS:
            # random data exists only for the same model
            data_model = run_model
            data_model_dir = DATA_ROOT / DB / "delete" / "random" / data_model
            ...
            for ratio_dir in sorted_dirs(data_model_dir):
                for seed_dir in sorted_dirs(ratio_dir):
                    ...
                    print(f"DB={DB} run_model={run_model} ratio={ratio} seed={seed}")
