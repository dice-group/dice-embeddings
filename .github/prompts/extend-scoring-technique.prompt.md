---
name: extend-scoring-technique
description: "Add a new scoring technique (training dataset class) to the dicee framework. Use when: implementing a new negative sampling strategy, creating a custom labelling scheme, adding a new loss formulation that requires a different dataset structure."
mode: agent
---
...
**Key files:**
- [`dicee/dataset_classes/_factory.py`](../../../dicee/dataset_classes/_factory.py) — `construct_dataset()` dispatcher
- [`dicee/dataset_classes/_negative_sampling.py`](../../../dicee/dataset_classes/_negative_sampling.py) — `NegSample`, `FixedNegSample` (reference for triple-level labelling)
- [`dicee/dataset_classes/_label_based.py`](../../../dicee/dataset_classes/_label_based.py) — `KvsAll`, `1vsAll`, `AllvsAll` (reference for entity-level labelling)
- [`dicee/models/base_model.py`](../../../dicee/models/base_model.py) — `BaseKGE.forward_triples` and `BaseKGE.forward_k_vs_all` contracts
...
Choose the shape that matches your loss semantics.
...
```python
# Locate the correct position in the elif chain in _factory.py:
...
The model must implement the forward method that matches your dataset's output shape.
