    ...
    try:
        ...
        with attached_context(model, data.context), evaluation_mode(model):
            ...
            inference = dict(method='cqd-global-prefix', model=model.name, beam_size=beam_size, tnorm=tnorm,
                             ...
                             adapter=dict(feature_mode=transform.feature_mode, observed_mix=transform.observed_mix,
                                          weights=transform.weights.detach().cpu().tolist(), metadata=transform.metadata),
