...
class BaseInteractiveKGE:
    ...
    def __init__(self, path: str = None, url: str = None, construct_ensemble: bool = False, model_name: str = None,
        ...
        #################################################################################################################
        if self.configs.get("byte_pair_encoding", None) and self.use_custom_tokenizer:
            ...
            self.dummy_id = Tokenizer.from_file("C:\\Users\\Harshit Purohit\\Byte\\myenv6\\Lib\\site-packages\\dicee\\Tokenizer\\Tokenizer_Path\\tokenizer.json").encode(" ").ids[0]
            self.max_length_subword_tokens = self.configs["max_length_subword_tokens"]
            if self.tokenizer_path:
                from tokenizers import Tokenizer
                self.enc = Tokenizer.from_file(tokenizer_path)
                self.dummy_id = Tokenizer.from_file(tokenizer_path).encode(" ").ids[0]
                self.max_length_subword_tokens = self.configs["max_length_subword_tokens"]
        elif self.configs.get("byte_pair_encoding", None) and not self.use_custom_tokenizer:
            import tiktoken
    ...
    def get_bpe_token_representation(self, str_entity_or_relation: Union[List[str], str]) -> Union[
        ...
        use_transformer_embeddings = self.use_transformer_embeddings
    ...
    def get_entity_embeddings(self, items: List[str]):
        ...
        tokenizer_path = self.tokenizer_path
        ######################################################################################################################
        if self.configs["byte_pair_encoding"] and use_custom_tokenizer:
            t_encode = self.enc.encode_batch(items)
            simple_ids = [enc.ids for enc in t_encode]
            if len(simple_ids) != self.configs["max_length_subword_tokens"]:
                for i in range(len(simple_ids)):
                    simple_ids[i].extend(
                        [self.dummy_id for _ in range(self.configs["max_length_subword_tokens"] - len(simple_ids[i]))])
            return self.model.token_embeddings(torch.LongTensor(simple_ids)).flatten(1)
        elif self.configs["byte_pair_encoding"] and not use_custom_tokenizer:
            t_encode = self.enc.encode_batch(items)
