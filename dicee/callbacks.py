...
class PPE(AbstractPPECallback):
    """
    Polyak Parameter Ensemble (PPE) callback for maintaining a running average of model parameters.

    This callback implements the Polyak Parameter Ensemble technique, which maintains
    ...
    ----------
    num_epochs : int
        Total number of epochs in the training process.
    path : str
        Path where the ensemble model and intermediate states are stored.
    epoch_to_start : int, optional
        Epoch number from which to start applying PPE. If not specified, PPE will start from the beginning.
    last_percent_to_consider : float, optional
        Last X percent of the epochs to consider for PPE. This parameter is used if `epoch_to_start` is not provided.

    Attributes
    ----------
    epoch_count : int
        Counter to track the current epoch number.
    sample_counter : int
        Counter to track the number of samples (epochs) considered in the ensemble.
    """
    ...
    def initialize_ensemble(self, model: torch.nn.Module) -> dict:
        """
        Initializes the ensemble by saving the current model's state dict.

        Parameters
        ----------
        model : torch.nn.Module
            The model from which to initialize the ensemble.

        Returns
        -------
        dict
            The state dict of the model used to initialize the ensemble.
        """
        torch.save(model.state_dict(), f=f"{self.path}/trainer_checkpoint_main.pt")
        return torch.load(
            f"{self.path}/trainer_checkpoint_main.pt", torch.device(model.device)
        )

    def should_averaging_start(self) -> bool:
        """
        Checks whether parameter averaging should start based on the current epoch.

        Returns
        -------
        bool
            True if averaging should start, False otherwise.
        """
        return self.epoch_to_start <= self.epoch_count

    def get_ensemble_model(self, model: torch.nn.Module) -> dict:
        """
        Retrieves the ensemble model's state dict.
        ...
        -------
        dict
            The state dict of the ensemble model.
        """
    ...
    def update_ensemble(
        self, ensemble_state_dict: dict, current_model: torch.nn.Module
    ):
        """
        Updates the ensemble model's state dict with the current model's parameters.
        This method performs an in-place update of the ensemble model's state dict
        using the Polyak averaging formula.

        Parameters
        ----------
        ensemble_state_dict : dict
            The state dict of the ensemble model to be updated.
        current_model : torch.nn.Module
            The current model from which parameters are extracted to update the ensemble.
        """
        with torch.no_grad():
            for k, parameters in current_model.state_dict().items():
                if parameters.dtype == torch.float:
                    ...
                    ensemble_state_dict[k] = (
                        ensemble_state_dict[k] * self.sample_counter + parameters
                    ) / (1 + self.sample_counter)

    def on_train_epoch_end(self, trainer: Trainer, model: torch.nn.Module) -> None:
        """
        Called at the end of each training epoch to possibly update the ensemble model.
        This method checks if the conditions for starting parameter averaging are met
        and, if so, updates the ensemble model with the current model's parameters.
        ...
        if self.should_averaging_start():
            ...
            self.update_ensemble(
                ensemble_state_dict=ensemble_state_dict, current_model=model
            )
...
class FPPE(AbstractPPECallback):
    ...
    def __init__(self, num_epochs, path, last_percent_to_consider=None):
        ...
        lamb = 0.1
        self.alphas = exponential_function(
            np.arange(self.num_ensemble_coefficient), lam=lamb, ascending_order=True
        )
