...
class TestRegressionQmult:
    ...
    def test_k_vs_sample_regression(self):
        ...
        assert result['Val']['MRR'] >= .44
        assert result['Val']['H@1'] >= .29
        assert result['Val']['H@3'] >= .51
        assert result['Val']['H@10'] >= .74

        assert result['Test']['MRR'] >= .44
        assert result['Test']['H@1'] >= .29
        assert result['Test']['H@3'] >= .51
        assert result['Test']['H@10'] >= .74

    @pytest.mark.filterwarnings('ignore::UserWarning')
    def test_k_vs_sample_regression_and_kvs_all(self):
        args = argparse_default([])
        args.model = 'QMult'
        args.path_dataset_folder = 'KGs/UMLS'
        args.optim = 'Adam'
        ...
        assert result['Val']['MRR'] >= .32
        assert result['Val']['H@1'] >= .22
        assert result['Val']['H@3'] >= .35
        assert result['Val']['H@10'] >= .53