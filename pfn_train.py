...
from pfn_inference import evaluate
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for _batch_idx, (support, query, label) in enumerate(pbar, start=1):
            ...
            if report_flops and not flops_reported:
                try:
                    total_flops = _estimate_train_step_flops(
                        ...
                        support=support, query=query, label=label, device=device,
                    )
                    if total_flops is None:
                        ...
                        print(
                            ...
                            f"{run_total_flops:,} FLOPs over {total_steps:,} steps = {run_total_human}."
                        )
            ...
            pbar.set_postfix(bce=f"{batch_loss:.4f}", avg=f"{running_avg:.4f}", lr=f"{current_lr:.2e}")
    ...
    if eval_train_file and eval_test_file:
        ...
        model.to(device)
        model.eval()
        metrics = evaluate(model, train_file=eval_train_file, test_file=eval_test_file, device=device, context_size=context_size)
        model.cpu()
